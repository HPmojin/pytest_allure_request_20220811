...
def run():
    ...
    try:
        ...
        shutil.rmtree(setting["enclosures"])#删除压缩打包的测试报告
    except:
        pass
    ...
    cmd_server=r'cp config/Start_server.bat target' #拷贝命令
    subprocess.call(cmd_server, shell=True)  # 启动服务器脚本，由config目录拷贝到target目录下进行压缩打包发送邮件
